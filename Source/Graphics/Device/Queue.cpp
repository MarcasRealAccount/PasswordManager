...
namespace Graphics {
	QueueFamily::QueueFamily(Device& device, std::uint32_t familyIndex, VkQueueFlags queueFlags, std::uint32_t timestampValidBits, VkExtent3D minImageTransferGranularity, bool supportsPresent, std::uint32_t queueCount)
		...
		for (std::uint32_t i = 0; i < queueCount; ++i) {
			VkQueue queue;
			vkGetDeviceQueue(m_Device, familyIndex, i, &queue);
			m_Queues.emplace_back(*this, i, queue);
		}
	...
	Queue::Queue(QueueFamily& queueFamily, std::uint32_t index, VkQueue handle)
